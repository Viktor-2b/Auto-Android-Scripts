...
# ==========================================
print("正在尝试连接手机...")
# "android:///" 表示自动连接当前电脑上唯一一台通过 USB 开启调试的安卓设备
...
# 2. 初始化 UI 树解析服务 (poco)
# ==========================================
print("正在初始化 poco UI 服务，初次运行需要点击同意安装 PocoService...")
...
print("--- 正在扫描首页拦截弹窗 ---")
# 弹窗 1：未付订单右上角的 X
unpaid_close_btn = poco("com.jingyao.easybike:id/ivClose")
# 弹窗 2：活动广告底部的 X
activity_close_btn = poco("com.jingyao.easybike:id/actionDialogClose")

if unpaid_close_btn.exists():
    print("⚠️ 发现【未付订单】！正在点击右上角的 'X' 按钮关闭...")
    unpaid_close_btn.click()
    sleep(0.5)
elif activity_close_btn.exists():
    print("⚠️ 发现【活动广告】弹窗！正在点击底部的 'X' 按钮关闭...")
    activity_close_btn.click()
    sleep(0.5)
else:
    print("✅ 未检测到拦截弹窗，直接在首页继续操作。")

print("--- 开始执行路径跳转，我的->奖励金 ---")

my_tab = poco(text="我的")
if my_tab.wait(10).exists():
    print("✅ 找到底部【我的】标签，正在点击...")
    my_tab.click()
    sleep(1.0)
else:
    raise Exception("❌ 未能找到【我的】标签，请检查界面状态。")

reward_btn = poco(text="奖励金")
if reward_btn.wait(10).exists():
    print("✅ 找到【奖励金】入口，正在点击...")
    reward_btn.click()
    print("🎉 成功点入奖励金页面！等待新页面加载...")
    sleep(2.0)
else:
    raise Exception("❌ 未能在'我的'页面找到【奖励金】入口。")
