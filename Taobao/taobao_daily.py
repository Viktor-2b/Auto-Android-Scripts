...
def get_taobao_gold_coins(d):
    click_and_wait(d, description="领淘金币")
    click_and_wait(d, text="签到领金币")
    click_and_wait(d, "back")


def switch_taobao_account(d):
    click_and_wait(d, description="我的淘宝")
    click_and_wait(d, description="设置")
    click_and_wait(d, description="切换账号")
    click_and_wait(d, text="切换")
...
def buy_88vip_daily_item(d):
    ...
    if not d(description=VIP_NAME).wait(timeout=3.0):
        ...
        return
    click_and_wait(d, description="收藏")
    click_and_wait(d, pos=(0.208, 0.280))  # 收藏商品
    click_and_wait(d, text="立即购买")
